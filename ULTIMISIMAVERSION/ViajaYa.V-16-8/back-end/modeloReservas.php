	...
	public function create( $Booking_data = array() ){
	   ...
	   public function delete($id_res = ''){
		...
		public function imprimir() {
			...
			FROM
				reservas r
			LEFT JOIN
				pasajeros p ON r.id_reservas = p.nro_reserva
			ORDER BY
				r.id_reservas ;";
	 ...
	 public function imprimirpasajeros(){
			...
			LEFT JOIN
				pasajeros p ON r.id_reservas = p.nro_reserva
			WHERE
				p.nro_reserva = r.idreservas;
			;";
		
		}
		
	
	public function obtenerCantidadPasajeros() {
		$this->query = "SELECT r.cantPasajeros AS cantidad_pasajeros
						FROM reservas r
						ORDER BY r.id_reservas DESC
						LIMIT 1";
		$this->get_query();
	
		if (!empty($this->rows) && isset($this->rows[0]['cantidad_pasajeros'])) {
			$cantidad = $this->rows[0]['cantidad_pasajeros'];
		} else {
			$cantidad = 0; 
		}
	 ...
	 function obtener_idUltReserva() {
		...
		$this->query= "SELECT r.id_reservas AS ID_reservas
					  FROM reservas r
					  ORDER BY r.id_reservas DESC
					  LIMIT 1";
		  $this->get_query();
		  if (!empty($this->rows)) {
		   $idpasajeros = $this->rows[0]['ID_reservas'];
	   } else {
		   $idpasajeros = "No se encontraron resultados";
	   }
	 
	   return $idpasajeros;
	 }
	 public function nuevopasajero($pasajero_data = array()){
		foreach ($pasajero_data as $key => $value) {
			$$key = $value;
			$$$key = $value;
			$$$$key = $value;
			$$$$$key = $value;
		...
		$this->query = "INSERT INTO pasajeros (nombre, apellido, dni, num_boleto, nro_reserva ) VALUES 
		( '$nombre', '$apellido', '$dni', '$num_boleto','$nro_reserva')";
		$this->set_query();
	} 
